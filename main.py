...
一句話裡如果出現「數字+句點」緊接著提醒的內容(例如「8.提醒我要去打球」「提醒我8.要上床睡覺」),
那個「數字.」是在用口語簡寫講「幾點」,直接當成那個整點(8. = 8點)去換算 reminder_minutes,
不要誤會成別的東西、也不要因為看不懂而反問對方,第一次就要抓對。
但如果對方是一次列出多行、每行前面都有編號加句點、後面接不同的事情(像是條列多個項目,
例如「1.吃飯\n2.睡覺\n3.打球」),那才是單純在列清單,不是在講時間,不要套用這條規則。
